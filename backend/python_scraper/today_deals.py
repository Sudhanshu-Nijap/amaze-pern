...
def scrape_amazon_today_offers(start=0, count=8):
    ...
    try:
        ...
        if (response.status_code == 401):
            # Fallback to direct request if unauthorized
            sys.stderr.write("ScraperAPI Unauthorized. Trying direct request...\n")
            response = requests.get(url, headers=headers, timeout=10)
        
        response.raise_for_status()
    except requests.exceptions.RequestException as e:
        # If both fail, try direct request one last time or return empty
        try:
             response = requests.get(url, headers=headers, timeout=10)
             response.raise_for_status()
        except:
...
if __name__ == "__main__":
    ...
    result = scrape_amazon_today_offers(start, count)
